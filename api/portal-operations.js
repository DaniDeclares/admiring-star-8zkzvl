...
// Mints a confirmation token for a newly scheduled appointment, stores its
// hash on the appointment row, and enqueues a branded confirmation email
// through the existing dd_event_outbox -> Resend pipeline. Never invoked for
// appointments created directly by migration/SQL, so this never fires for a
// customer whose confirmation was already sent manually.
...
async function getStaffSnapshot(supabase) {
  ...
  // Fetched separately and tolerated as empty/null: both tables ship in a
  // migration this environment cannot apply, so the console must keep working
  // whether or not that migration has run yet in production.
