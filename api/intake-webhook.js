...
export default async function handler(req, res) {
  ...
  try {
    ...
    const lead = await prisma.lead.create({
      data: { full_name: name, email: email || null, phone: phone || null, organization_name: organizationName || null, status: 'new', notes: null },
    });
    ...
    const notificationText = [
      'New DANI DECLARES service request',
      `Name: ${name}`,
      `Email: ${email || 'not provided'}`,
      `Phone: ${phone || 'not provided'}`,
      `Channel: ${routing.channel}`,
      `Service: ${serviceType || category || 'not specified'}`,
      `Service ID: ${canonicalServiceId || 'not specified'}`,
    ...
    if (process.env.NOTIFICATION_EMAIL) {
      await publishOperationalEvent({
        ...
        eventKey: `lead-created-email:${request.id}`,
        channel: 'EMAIL',
        payload: {
    ...
    if (process.env.NOTIFICATION_PHONE) {
      await publishOperationalEvent({
        ...
        channel: 'SMS',
        payload: { to: process.env.NOTIFICATION_PHONE, text: `New DANI DECLARES lead: ${name}; ${serviceType || category || 'service request'}; ${phone || email || ''}; Request ${request.id}` },
      });
    }
    ...
    console.error('Intake Webhook Persistence/Notification Error:', error);
