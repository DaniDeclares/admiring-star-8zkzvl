...
export default async function handler(req, res) {
  ...
  try {
    ...
    const amount = resolveCommercialPrice({
      ...
      residentSubchannel: subchannel,
      isVerifiedResident: channel === 'CH01' && subchannel === 'CH01-A' && body.isResident === true,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        price_data: {
          ...
          product_data: { name: record.name, metadata: { service_id: serviceId, channel, subchannel } },
          ...(recurring ? { recurring: { interval: 'month' } } : {}),
        },
        quantity: 1,
      }],
      metadata: { request_id: requestId, service_id: serviceId, channel, subchannel, commercial_relationship: record.channel },
      payment_intent_data: recurring ? undefined : { metadata: { request_id: requestId, service_id: serviceId, channel, subchannel } },
    ...
    return json(res, 400, { error: error?.message?.startsWith('Commercial Block:') ? error.message : 'Secure checkout could not be opened. Please try again or contact DANI DECLARES.' });
  }
}
