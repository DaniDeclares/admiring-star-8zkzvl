...
export default async function handler(req,res){
 ...
 try{
  ...
  const subchannel=String(request.property_details?.operationsRouting?.subchannelCode||body.subchannelCode||'').trim();
  ...
  const governedAmount=Number(gate.price);
  ...
  const params={
   ...
   line_items:[{price_data:{currency:'usd',unit_amount:Math.round(frozenAmount*100),product_data:{name:offer.name,metadata:{service_id:serviceId,request_id:requestId,estimate_id:estimate.id,channel,subchannel}},...(recurring?{recurring:{interval:'month'}}:{})},quantity:1}],
   metadata:{request_id:requestId,service_id:serviceId,estimate_id:estimate.id,channel,subchannel},
   ...(recurring?{}:{payment_intent_data:{metadata:{request_id:requestId,service_id:serviceId,estimate_id:estimate.id,channel,subchannel}}}),
