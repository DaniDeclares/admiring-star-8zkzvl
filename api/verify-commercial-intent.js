  ...
  try {
    if (req.method === 'GET' && req.query?.catalog === '1') {
      const rows = await prisma.$queryRawUnsafe(`
        SELECT s.sku AS "serviceId", s.name, LPAD(s.division_id::text,2,'0') AS division,
               s.service_family AS family, s.starting_price AS "baseCustomerPrice",
               ...
               s.pricing_type AS "pricingType", s.billing_cycle AS "billingCycle",
               s.resident_discount_eligible AS "residentDiscountEligible", s.commercial_status AS status
    ...
    if(!record){
      const rows=await prisma.$queryRawUnsafe(`SELECT sku,name,starting_price,base_price_cents,pricing_type,billing_cycle,resident_discount_eligible,commercial_status,canonical_notes FROM public.services WHERE sku=$1 LIMIT 1`,serviceId);
      const db=rows?.[0];
      if(db) record={serviceId:db.sku,name:db.name,division:String(serviceId).slice(4,6),model:db.pricing_type==='SOW'||db.pricing_type==='SOW_PROCUREMENT'?'BESPOKE_SOW':'FIXED_FLAT',baseCustomerPrice:db.starting_price==null?null:Number(db.starting_price),billingCycle:db.billing_cycle,residentDiscountEligible:Boolean(db.resident_discount_eligible),stripeExecutionMode:db.pricing_type==='SOW'||db.pricing_type==='SOW_PROCUREMENT'?'MANUAL_INVOICE':'DYNAMIC_CHECKOUT',providerIsolationLane:'SPECIALIST_NETWORK',status:db.commercial_status,canonicalNotes:db.canonical_notes};
    }
