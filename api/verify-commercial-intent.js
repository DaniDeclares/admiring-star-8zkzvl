...
export default async function handler(req,res){
 try{
  ...
  if(!record){
   ...
   const db=rows?.[0];
   if(db)record={serviceId:db.sku,name:db.name,division:db.division,model:String(db.pricing_type||'').toUpperCase(),baseCustomerPrice:db.starting_price==null?null:Number(db.starting_price),billingCycle:db.billing_cycle,residentDiscountEligible:Boolean(db.resident_discount_eligible),status:db.commercial_status};
  }
  if(!record||record.status!=='CANONICAL_ACTIVE')return json(res,404,{error:'This service is not currently available to request online.'});
  ...
  if(registryRecord){
   ...
   if(Boolean(body.isResident)&&record.residentDiscountEligible&&expectedPrice!=null)expectedPrice=Math.round(Number(expectedPrice)*.85*100)/100;
  }
