...
export default async function handler(req,res){
 try{
  if(req.method==='GET'&&req.query?.catalog==='1'){
   ...
   const ownerOnlyLiveSkus=new Set(['DNI-01A-009','DNI-01A-010','DNI-01D-002','DNI-01D-004']);
   return json(res,200,{success:true,count:rows.length,services:rows.map(row=>{const liveOwner=ownerOnlyLiveSkus.has(row.serviceId);return {...row,baseCustomerPrice:row.baseCustomerPrice==null?null:Number(row.baseCustomerPrice),orderableNow:row.commercialOfferStatus==='SELL_NOW'&&row.fulfillmentGateStatus==='READY',ownerExecutableLive:liveOwner,intakeAvailable:true};})});
  }
