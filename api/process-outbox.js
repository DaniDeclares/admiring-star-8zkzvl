...
async function sendEmail(payload) {
  ...
  const response = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from, to: payload.to, subject: payload.subject || 'Dani Declares Update', html: payload.html || `<p>${payload.text || ''}</p>` }),
  });
  ...
  try {
    ...
    for (const item of items) {
      try {
        ...
        await prisma.$executeRaw`
          update public.dd_event_outbox
          set status = 'FAILED', last_error = ${String(error.message || error)}, available_at = now() + (${delayMinutes} || ' minutes')::interval, updated_at = now()
          where id = ${item.id}::uuid
        `;
