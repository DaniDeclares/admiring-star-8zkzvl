...
function evaluateResearchSignals(text, signals) {
  ...
  return (Array.isArray(signals) ? signals : []).map(signal => {
    ...
    const allMatched = all.every(term => haystack.includes(String(term).toLowerCase()));
    const anyMatched = any.length === 0 || any.some(term => haystack.includes(String(term).toLowerCase()));
    return {
      signal: signal?.signal || 'unnamed',
...
async function processResearchSources() {
  // The scheduler may call this endpoint every five minutes; each source has its
  // own next_check_at so ordinary sources are fetched only every 6-24 hours.
  ...
  for (const source of sources) {
    ...
    try {
      // Claim the row immediately so overlapping cron invocations do not refetch it.
      ...
      const decoded = isText ? buffer.toString('utf8') : '';
      const cleanText = isText ? cleanResearchText(decoded) : '';
      ...
      if (source.work_key) {
        await prisma.$executeRaw`
          ...
          set attempts = attempts + 1, last_researched_at = now(),
              updated_at = now(),
              next_action = case
      ...
      // Deterministic signal evidence is useful, but it never turns the parent
      // regulated work item GREEN. Current primary/regulator/vendor pages may
      // confirm a narrow claim; historical contracts stay HISTORICAL.
      ...
      results.push({ sourceKey: source.source_key, status: 'FAILED', error: message });
    }
  }
  return results;
}
