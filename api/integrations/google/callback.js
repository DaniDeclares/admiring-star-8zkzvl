...
export default async function handler(req,res){
 ...
 try{
  ...
  const stateRow=await consumeOAuthState({supabase,state,adapterCode:'GMAIL'});
  ...
  for(const [adapterCode,required] of Object.entries(ADAPTER_SCOPES)){
   ...
   await logIntegrationEvent({supabase,adapterCode,connectionId:connection.id,direction:'INBOUND',eventType:'OAUTH_CONNECTED',status:'PROCESSED',payload:{google_subject:me.sub,email:me.email||null}});
  }
