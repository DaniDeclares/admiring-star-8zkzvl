...
export default async function handler(req,res){
 ...
 try{
  ...
  const {data:connections,error}=await context.supabase.from('dd_integration_connections').select('adapter_code,environment,external_account_id,connection_status,permissions,token_expires_at,last_sync_at,last_error,updated_at').eq('environment', ENVIRONMENT).in('adapter_code',['ASANA','NOTION','QUICKBOOKS_ONLINE','GOOGLE_VOICE','GMAIL','GOOGLE_CALENDAR','GOOGLE_DRIVE','HUBSPOT','AIRTABLE','GITHUB','VERCEL','POSTHOG','PAYPAL_BUSINESS']).order('updated_at',{ascending:false});
