...
export default async function handler(req,res){
 ...
 try{
  ...
  if(process.env.NOTION_TOKEN){try{const r=await fetch('https://api.notion.com/v1/users/me',{headers:{Authorization:'Bearer '+process.env.NOTION_TOKEN,'Notion-Version':'2026-03-11'}});notionInternalValid=r.ok;}catch(_){notionInternalValid=false;}}
  const env={
 ...
 }catch(error){return res.status(error.status||500).json({success:false,error:error.message||'INTEGRATION_STATUS_FAILED'});}
}