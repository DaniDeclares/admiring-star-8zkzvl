...
Service requests submitted through `/request-service` are persisted to `leads` and `service_requests`. The intake webhook now publishes notification intents to the transactional outbox after successful persistence. Operator email is queued when `NOTIFICATION_EMAIL` is configured; customer confirmation email is queued to the submitted customer email independently of the operator inbox. SMS is queued when `NOTIFICATION_PHONE` is configured. Notification delivery is handled by `api/process-outbox.js`.
## Delivery
...
Every successful web intake produces a separate `CUSTOMER_REQUEST_RECEIVED` email intent for the submitted customer email (unless it is the same as the operator inbox, to avoid duplicate delivery). The confirmation contains the request ID, requested timing/location, and the explicit statement that requested timing is not a final appointment until DANI DECLARES confirms scope, availability and scheduling.
