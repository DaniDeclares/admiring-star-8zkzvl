...
export default function PortalLoginPage(){
 ...
 const submit=async e=>{e.preventDefault();setBusy(true);setError('');const normalized=email.trim().toLowerCase();const {error}=await supabase.auth.signInWithPassword({email:normalized,password});if(error){setError(error.message);setBusy(false);return;}navigate(normalized==='vendors@danideclares.com'?'/portal/change-password':'/portal',{replace:true});};
