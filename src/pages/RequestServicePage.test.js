...
describe('public request front door regressions',()=>{
 ...
 test('household mode does not render an organization field',()=>{expect(source).toContain('form.channelType!==OPERATIONS_CHANNELS.B2C');expect(source).toContain("organizationName:e.target.value===OPERATIONS_CHANNELS.B2C?'':f.organizationName");});
 test('needs-based intake does not require a preselected service',()=>{expect(source).toContain("pricingServiceId:form.serviceId||null");expect(source).toContain("'General service request'");expect(source).toContain('if(form.serviceId&&form.frontDoorCode)');});
