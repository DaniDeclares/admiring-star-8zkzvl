...
export default function PropertyPage() {
  ...
  useEffect(()=>{fetch('/api/verify-commercial-intent?catalog=1&channelType=B2B_APT').then(r=>r.json()).then(d=>{if(d.success)setCatalog(d.services||[]);}).catch(()=>{});},[]);
  const featured = useMemo(()=>new Map(catalog.map(s=>[s.serviceId,s])),[catalog]);
  const featuredSolutions=[
    {sku:'DNI-02A-026',label:'Turnover Ready Standard',outcome:'A coordinated, documented unit turn for leasing and operations teams.',cta:'Get Quote Now'},
    {sku:'DNI-02A-027',label:'Turnover Ready Premium',outcome:'Priority turnover support with kickoff, vendor coordination, progress updates and a management-ready final report.',cta:'Get Quote Now'}
  ];
  const otherServices=catalog.filter(s=>String(s.division)==='02' && !featuredSolutions.some(x=>x.sku===s.serviceId)).filter(s=>s.intakeAvailable);
  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", color: "#211417", background: "#fbf8f4", minHeight: "100vh" }}>
      ...
      <main style={{ maxWidth: 1080, margin: "0 auto", padding: "3.5rem 1.5rem 5rem" }}>
        <section style={{ marginBottom: 48 }}>
          ...
          <p style={{ maxWidth: 760, color: "#66565a", lineHeight: 1.7 }}>Choose the scope that matches the immediate problem. DANI carries the customer relationship, property context and CH02 commercial routing into the quote request so the operator does not have to rebuild the request by hand.</p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(300px,1fr))", gap: 16, marginTop: 22 }}>
            {featuredSolutions.map(item=>{const s=featured.get(item.sku);return <article key={item.sku} style={{ background: "white", border: "2px solid #d8bd78", borderRadius: 14, padding: 24 }}>
              ...
              <div style={{ marginTop: 14, fontWeight: 900, color: "#5b1424" }}>{s?.baseCustomerPrice!=null ? '
...
+Number(s.baseCustomerPrice).toFixed(2)+(s?.model==='FIXED'?'':' starting') : 'Quote required'}</div>
              <Link to={'/request-service?channelType=B2B_APT&service='+encodeURIComponent(item.sku)} style={{ display:"inline-flex",marginTop:16,background:"#6b1426",color:"white",padding:"12px 18px",borderRadius:7,textDecoration:"none",fontWeight:900 }}>{item.cta}</Link>
            </article>})}
          </div>
        </section>
