...
export default function PortalWorkspacePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (!data.session) { setError('Please sign in to access this portal.'); setLoading(false); return; }
  ...
  return <main className="portal-shell">
    ...
    {isProvider ? <>
      ...
      <Card title="Invoices & Financial Records"><p className="portal-note">Invoices display finalized financial records. This portal never recalculates catalog pricing.</p>{snapshot.invoices?.length ? snapshot.invoices.map(item => <div className="portal-row" key={item.id}><div><strong>{item.public_reference}</strong><small>{item.invoice_status} · Balance: ${Number(item.balance_due || 0).toFixed(2)}</small></div></div>) : <Empty>No invoices are currently attached to this workspace.</Empty>}</Card>
    </>}
