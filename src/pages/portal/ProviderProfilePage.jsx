...
export default function ProviderProfilePage() {
  ...
  return <main className="portal-shell">
    ...
    <Card title="Documents on File">{documents.length ? documents.map(item => <div className="portal-row" key={item.id}><div><strong>{item.document_type.replaceAll('_', ' ')}</strong><small>{statusLabel(item.verification_status)} · Uploaded {formatDate(item.uploaded_at)}</small></div></div>) : <Empty>No documents uploaded yet.</Empty>}<div className="portal-actions" style={{ marginTop: 14 }}><Link className="portal-primary" to="/portal/vendor-onboarding">Manage documents</Link></div></Card>
  </main>;
}
