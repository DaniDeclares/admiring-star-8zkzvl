...
function OperationsConsole() {
  ...
  const instantiateFulfillment = async (requestId) => {
    setMessage(''); setError('');
    const { data: auth } = await supabase.auth.getSession();
    if (!auth.session) return setError('Staff session required.');
    const response = await fetch('/api/portal-fulfillment', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${auth.session.access_token}` }, body: JSON.stringify({ requestId }) });
    const body = await response.json();
    if (!response.ok || !body.success) setError(body.error || 'Fulfillment could not be instantiated.');
    else { setMessage(`Fulfillment instantiated: ${body.fulfillment.work_order_id} → ${body.fulfillment.job_id}`); await load(); }
  };

  const routeDispatch = async (jobId) => {
    setMessage(''); setError('');
    const { data: auth } = await supabase.auth.getSession();
    if (!auth.session) return setError('Staff session required.');
    const response = await fetch('/api/portal-dispatch', { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${auth.session.access_token}` }, body: JSON.stringify({ jobId }) });
    const body = await response.json();
    if (!response.ok || !body.success) setError(body.error || 'Dispatch routing failed.');
    else { setMessage(body.routing?.offer_status === 'OFFERED' ? 'Provider offer created through the canonical dispatch resolver.' : `Dispatch held: ${body.routing?.routing_reason || body.routing?.offer_status || 'No eligible provider.'}`); await load(); }
  };
  ...
  return <main className="ops-shell">
    ...
    <section className="ops-grid"><aside className="ops-nav"><div className="ops-nav-title">WORKFLOWS</div>{QUEUES.map(item => <button key={item.key} className={active === item.key ? 'active' : ''} onClick={() => setActive(item.key)}><span>{item.label}</span><small>{counts[item.key]}</small></button>)}</aside>
      <section className="ops-workspace"><div className="ops-section-head"><div><p className="ops-eyebrow">LIVE OPERATIONAL LAYER</p><h2>{activeQueue.label}</h2><p>{activeQueue.description}</p></div><span className="ops-live">● STAFF ACCESS</span></div>
        {active === 'requests' && <><div className="ops-card"><h3>Channel routing</h3><div className="channel-grid">{CHANNELS.map(channel => <div key={channel}><strong>{channel}</strong><span>{channelCounts[channel]}</span></div>)}</div></div><div className="ops-card"><h3>Recent intake</h3>{(data?.requests || []).slice(0,8).map(row => <div className="ops-row" key={row.id}><div><strong>{row.service_needed || row.service_category || 'Service request'}</strong><small>{row.status} · {row.location_address || 'Location on file'}</small></div><div className="ops-actions">{row.service_id && row.division_id && row.status !== 'completed' && !(data?.jobs || []).some(job => job.service_request_id === row.id) && <button onClick={() => instantiateFulfillment(row.id)}>Instantiate fulfillment</button>}</div></div>)}</div></>}
