...
export default function Navbar() {
  ...
  return <header className="dd-navbar-header"><div className="dd-navbar-container">
    ...
    <div className="dd-navbar-actions">{session?<><Link to="/portal" className="dd-top-link" onClick={closeAll}>My Portal</Link><button type="button" className="dd-top-link" onClick={signOut}>Sign Out</button></>:<Link to="/portal/login" className="dd-top-link" onClick={closeAll}>Sign In</Link>}<Link to="/request-service" className="dd-project-cta" onClick={closeAll}>Book / Request</Link><button type="button" className="dd-mobile-toggle" onClick={()=>setMobileOpen(!mobileOpen)} aria-label="Toggle menu">{mobileOpen?'✕':'☰'}</button></div>
