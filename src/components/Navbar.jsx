  ...
  return <header className="dd-navbar-header">
    <div className="dd-navbar-container">
      ...
      <nav className="dd-navbar-nav desktop-only" aria-label="Primary navigation">
        ...
        {groups.map(group=><div className="dd-nav-group" key={group.key}>
          <button type="button" className={`dd-top-link dd-nav-trigger ${openMenu===group.key?'is-open':''}`} onClick={()=>setOpenMenu(openMenu===group.key?null:group.key)} aria-expanded={openMenu===group.key}>{group.label} <span>▾</span></button>
          {openMenu===group.key&&<div className="dd-mega-menu single-column">{group.links.map(([label,path])=><Link key={label} to={path} onClick={closeAll} className="dd-mega-link">{label}</Link>)}</div>}
        </div>)}
      ...
      <div className="dd-navbar-actions">
        <Link to="/request-service" className="dd-project-cta" onClick={closeAll}>Book / Request</Link>
        <button type="button" className="dd-mobile-toggle" onClick={()=>setMobileOpen(!mobileOpen)} aria-label="Toggle menu">{mobileOpen?'✕':'☰'}</button>
      </div>
    </div>
    {mobileOpen&&<div className="dd-mobile-menu">
      <Link to="/" onClick={closeAll} className="dd-mobile-home">Home</Link>
      <Link to="/catalog" onClick={closeAll} className="dd-mobile-home">Services</Link>
      {groups.map(group=><div className="dd-mobile-group" key={group.key}><div className="dd-mobile-group-title">{group.label}</div>{group.links.map(([label,path])=><Link key={label} to={path} onClick={closeAll}>{label}</Link>)}</div>)}
      ...
      <Link to="/request-service" onClick={closeAll} className="dd-mobile-project-cta">Book / Request Service</Link>
    </div>}
  </header>;
}
