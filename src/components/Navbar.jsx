...
export default function Navbar() {
  ...
  return <header className="dd-navbar-header"><div className="dd-navbar-container">
    ...
    <nav className="dd-navbar-nav desktop-only" aria-label="Primary navigation">
      ...
      <NavLink className="dd-top-link" to="/solutions">What We Handle</NavLink>
    ...
    <div className="dd-navbar-actions">{session?<><Link to="/portal" className="dd-top-link" onClick={closeAll}>My Portal</Link><button type="button" className="dd-top-link" onClick={signOut}>Sign Out</button></>:<Link to="/portal/login" className="dd-top-link" onClick={closeAll}>Sign In</Link>}<Link to="/campaign/handled" className="dd-top-link" onClick={closeAll}>More Gets Handled</Link><Link to="/request-service" className="dd-project-cta" onClick={closeAll}>Book / Request</Link><button type="button" className="dd-mobile-toggle" onClick={()=>setMobileOpen(!mobileOpen)} aria-label="Toggle menu">{mobileOpen?'✕':'☰'}</button></div>
  </div>{mobileOpen&&<div className="dd-mobile-menu"><Link to="/" onClick={closeAll} className="dd-mobile-home">Home</Link><Link to="/catalog" onClick={closeAll} className="dd-mobile-home">Services</Link><Link to="/solutions" onClick={closeAll} className="dd-mobile-home">What We Handle</Link><div className="dd-mobile-group-title dd-mobile-section-heading">Who We Serve</div>{groups.map(group=><div className="dd-mobile-group" key={group.key}><div className="dd-mobile-group-title">{group.label}</div>{group.links.map(([label,path])=><Link key={label} to={path} onClick={closeAll}>{label}</Link>)}</div>)}<Link to="/about" onClick={closeAll} className="dd-mobile-home">About</Link><Link to="/contact" onClick={closeAll} className="dd-mobile-home">Contact</Link>{session?<><Link to="/portal" onClick={closeAll} className="dd-mobile-home">My Portal</Link><button type="button" onClick={signOut} className="dd-mobile-home">Sign Out</button></>:<Link to="/portal/login" onClick={closeAll} className="dd-mobile-home">Sign In</Link>}<Link to="/request-service" onClick={closeAll} className="dd-mobile-project-cta">Book / Request Service</Link></div>}</header>;
}
