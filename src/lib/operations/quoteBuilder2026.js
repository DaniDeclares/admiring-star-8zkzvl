...
export async function createEstimate(supabase, body) {
  ...
  const { data: offer, error: offerError } = await supabase.from('dd_governed_service_offers').select('canonical_sku,service_name,division,commercial_offer_status,fulfillment_gate_status,runtime_service_id').eq('canonical_sku',serviceSku).neq('commercial_offer_status','DO_NOT_SELL').order('commercial_offer_status',{ascending:true}).limit(1).maybeSingle();
  if(offerError) throw offerError;
  if(!offer?.runtime_service_id) throw new Error('That service is not currently quoteable.');
  const { data: service, error: serviceError } = await supabase.from('services').select('*').eq('id',offer.runtime_service_id).maybeSingle();
  if(serviceError) throw serviceError;
  if(!service) throw new Error('The service record could not be resolved.');
  ...
  const payload={public_reference:publicReference,division_slug:String(service.division_id).padStart(2,'0'),source_slug:'admin_quote_builder',client_name:String(body.clientName||'').trim()||null,client_phone:String(body.clientPhone||'').trim()||null,client_email:String(body.clientEmail||'').trim()||null,client_type:ESTIMATE_CLIENT_TYPES[clientType]||'other',organization_name:String(body.organizationName||'').trim()||null,location_address:String(body.locationAddress||'').trim()||null,city:String(body.city||'').trim()||null,state:String(body.state||'GA').trim().toUpperCase()||null,zip_code:String(body.zipCode||'').trim()||null,timeline:String(body.timeline||'').trim()||null,rush_requested:Boolean(answers.rush),requested_date:body.requestedDate||null,intake_answers:{serviceSku,serviceName:offer.service_name,originalClientType:clientType,channelCode,answers,pricingSnapshot:{capturedAt:new Date().toISOString(),pricingRuleId:rule?.id||null,lockStatus:rule?.lock_status||null,...calculation}},client_notes:String(body.clientNotes||'').trim()||null,internal_notes:String(body.internalNotes||'').trim()||null,estimate_status:calculation.needsReview?'needs_review':'estimated',priority:String(body.priority||'normal'),base_subtotal:calculation.baseSubtotal,addon_subtotal:0,travel_fee:calculation.travelFee,rush_fee:calculation.rushFee,supplies_fee:calculation.sourcingFee+calculation.materials,pass_through_fee:calculation.passThrough,tax_amount:calculation.tax,estimated_total:calculation.estimatedTotal,deposit_due:calculation.depositDue,quote_disclaimer:'Estimate generated from the current DANI DECLARES commercial catalog and pricing rules. Final price remains subject to scope, location, materials/pass-throughs, fulfillment authorization, tax review and applicable service-specific gates.'};
  const { data:estimate,error:estimateError }=await supabase.from('dd_estimates').insert(payload).select('id,public_reference,estimate_status,estimated_total,deposit_due,quote_disclaimer').single();
