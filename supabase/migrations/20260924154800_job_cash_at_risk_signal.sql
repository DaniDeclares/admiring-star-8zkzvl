-- Job cash-at-risk signal — production
--
-- Gap: the Company Controller (dd_company_domain_state / dd_run_company_controller
-- / dd_generate_company_morning_brief, shipped in
-- 20260924133000_company_controller_morning_brief_production.sql) had no signal
-- combining a job's revenue, deposit, balance due, acquisition cost, recorded
-- fulfillment cost, and payment-evidence quality into one owner-facing
-- cash-at-risk item. That gap was surfaced by the 2026-09-24 Krystal Livingston
-- job (scheduled 3-5pm): $160 total / $50 Zelle deposit / $110 balance due /
-- $42.40 Thumbtack CAC were all sitting in dd_jobs.internal_notes as free text,
-- dd_job_cost_actuals had zero rows for any job, and nothing flagged that the
-- deposit's own payment event carries a self-contradictory note ("cleared" vs
-- "Chase screenshot shows pending").
--
-- This migration is purely additive:
--   1. Adds four nullable economics columns to dd_jobs (revenue_total,
--      deposit_expected, balance_due, acquisition_cost). No existing column
--      changes type or drops. Existing rows are untouched except one
--      documented backfill (below) of already-confirmed values for the
--      Krystal Livingston job, guarded so it never overwrites a value once set.
--   2. Adds dd_compute_job_cash_risk(), a new read-only function. It does not
--      invent a bank-balance feed (none is connected) — liquidity beyond what
--      dd_payment_events/dd_job_cost_actuals already record stays out of scope
--      for this function; it flags data gaps (untracked fulfillment cost,
--      short deposit, conflicting payment evidence) rather than fabricating
--      a number the system cannot verify.
--   3. Extends dd_refresh_company_domain_state() to fold that signal into the
--      existing ACCOUNTING_MONEY domain's metrics (which was already RED with
--      empty metrics) instead of inventing a new domain or a parallel table.
--      Every other domain's logic in this function is unchanged byte-for-byte.
--   4. Files one immediate dd_owner_attention_queue item for the Krystal job
--      so it is live on the dashboard now, not only after the next scheduled
--      controller run.
--   5. Runs the controller + regenerates today's brief once, in-transaction,
--      so the signal is live immediately (same "no external side effects"
--      pattern as the existing functions — DB-internal only).
--
-- Idempotent and safe to re-run.

begin;

-- 1. Additive job-economics columns -------------------------------------------
...
-- dd_job_cost_actuals already exists in production; this is a no-op there and
-- ports the same shape to any environment where it is still missing (e.g.
-- TESTER). estimate_component_snapshot_id intentionally has no FK here
-- (dd_estimate_component_snapshots doesn't exist in every environment yet);
-- production's own copy of that FK, already in place, is unaffected by this
...
-- Backfill only the one job whose figures are already confirmed in
-- internal_notes ("Negotiated and customer-confirmed total $160. Thumbtack
-- CAC $42.40. $50 Zelle deposit expected/pending verification; $110 balance
-- after verified deposit."). Guarded to never overwrite a value already set.
...
-- 2. Cash-at-risk computation function ------------------------------------------
...
-- 3. Fold the signal into the existing ACCOUNTING_MONEY domain -----------------
-- Every branch below is byte-for-byte identical to
-- 20260924133000_company_controller_morning_brief_production.sql except the
-- new v_cash_risk declaration/assignment and the new ACCOUNTING_MONEY case in
-- the `metrics` update (previously ACCOUNTING_MONEY fell through to `else
-- metrics`, i.e. stayed empty).
...
-- 4. File an immediate owner-attention item for the Krystal Livingston job -----
-- (idempotent: only inserts if there is no already-open item for this job)
...
-- 5. Refresh state + regenerate today's brief now (DB-internal only; no
--    external side effects, matching the existing controller functions).
