...
select
 ...
 case
   ...
   when m.service_family = '01C Indoor Plant Care' then 'SUPPORTING_LAYER'
   when m.service_family = '01B Pet Care & Household Pet Support' then 'SUPPORTING_LAYER'
   when m.service_family in ('01D Household Concierge','01E Move & Household Transition','01F Seasonal/Holiday') then 'SUPPORTING_LAYER'
   when m.service_family like '04A%' or m.service_family = 'Administrative & Business Operations' then 'OFFICE_OPERATIONS_RESCUE'
   when m.service_family = 'Experiences & Resident Programming' then 'SUPPORTING_LAYER'
   when m.service_family like 'Notary%' or m.service_family like '05A%' then 'CROSS_CHANNEL_REVIEW'
   when m.service_family like 'Real Estate%' then 'CROSS_CHANNEL_REVIEW'
 ...
 case
   ...
   when m.service_family like '04A%' or m.service_family = 'Administrative & Business Operations' then 'Property-office operations support candidate.'
   else 'Supporting capability; not a CH02 front door by default.'
 end,
 case when m.service_family in ('01C Indoor Plant Care','01B Pet Care & Household Pet Support','01D Household Concierge','01E Move & Household Transition','01F Seasonal/Holiday','Experiences & Resident Programming') then true else false end,
 ...
 true,true,true,
 'Initial CH02 adjudication generated from the locked CH02 strategy contract and the existing channel matrix. No source service, price, or storefront state changed.',
 ...
 case
   ...
   when m.service_family like 'Real Estate%' then 80
   when m.service_family like 'Notary%' or m.service_family like '05A%' then 90
   else 60
 end
...
create index if not exists idx_dd_ch02_adjudication_disposition
on public.dd_ch02_service_adjudication(disposition,priority);
create index if not exists idx_dd_ch02_adjudication_front_door
on public.dd_ch02_service_adjudication(proposed_front_door,priority);
