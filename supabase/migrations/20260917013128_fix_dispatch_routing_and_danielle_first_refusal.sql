-- BUG FIX: dd_route_work_order() filtered on permission_status='AUTHORIZED', a value that
-- does not exist anywhere in dd_provider_organizations (real values are PERMISSION_GRANTED,
-- APPROVED, PENDING). This made the function return NO_ELIGIBLE_PROVIDER unconditionally for
-- every single invocation ever made (confirmed: its one historical log row is exactly that).
-- Also adds: exclusion of orgs/providers that already REJECTED this same job, so a second
-- call (triggered automatically after a decline) offers to the NEXT eligible provider instead
-- of re-offering to someone who already said no.
...
-- Danielle Fong (owner/operator, provider org 19c10267-898f-4c10-a25e-186f6aff8771) gets
-- first right of refusal on every capability she is actually authorized for (CLEANING,
-- CONCIERGE, HOME_WATCH, LAUNDRY_VALET -- confirmed via dd_provider_capabilities; she holds
-- no NOTARY_PUBLIC capability, so the existing capability-authorization join in
-- dd_route_work_order already excludes her from notary work with no extra carve-out needed).
-- routing_priority=1 beats every other org's default of 100, so dd_route_work_order's
-- ORDER BY o.routing_priority, o.id always selects her first among eligible orgs.
UPDATE public.dd_provider_organizations SET routing_priority = 1 WHERE id = '19c10267-898f-4c10-a25e-186f6aff8771';
