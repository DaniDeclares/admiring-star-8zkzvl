...
--
-- These services require configuration inputs and a frozen estimate before payment.
-- The inactive $25 Stripe Payment Links are retained as audit evidence only; they must
-- not be activated as though $25 were the complete configured order price.

update public.services
set pricing_type = 'VARIABLE_QUOTE',
    updated_at = now()
...
set pricing_model = 'VARIABLE_QUOTE',
    updated_at = now()
...
begin
  ...
  where sku in ('DNI-11A-017', 'DNI-11A-018')
    and pricing_type = 'VARIABLE_QUOTE';

  select count(*)
  into v_quote_schema_count
  from public.services
  where sku in ('DNI-11A-017', 'DNI-11A-018')
    and quote_input_schema is not null
    and jsonb_typeof(quote_input_schema) = 'object';

  select count(*)
  into v_active_link_count
  from public.dd_stripe_launch_register
  where canonical_sku in ('DNI-11A-017', 'DNI-11A-018')
    and activation_decision in ('ACTIVATE', 'ACTIVE');

  if v_service_count <> 2 then
    raise exception 'D11 configured-payment classification expected 2 services, found %', v_service_count;
  end if;

  if v_quote_schema_count <> 2 then
    raise exception 'D11 configured-payment release requires 2 governed quote schemas, found %', v_quote_schema_count;
  end if;

  if v_active_link_count <> 0 then
    raise exception 'D11 fixed Payment Links must remain inactive until the configured quote path is replaced';
  end if;
end
$$;
