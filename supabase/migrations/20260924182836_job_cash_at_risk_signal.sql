-- Job cash-at-risk signal — production (PR #395, merge ff6b4d6)
begin;
...
select
  ...
  'Job has an unresolved deposit-evidence conflict and no fulfillment-cost tracking. Revenue: $' || dj.revenue_total ||
    ', balance due: $' || dj.balance_due || ', deposit expected: $' || dj.deposit_expected || ', Thumbtack CAC: $' || dj.acquisition_cost || '.',
  'URGENT', 'OPEN',
  'Verify the $50 Zelle deposit is actually available in the Chase app before relying on it (payment event has a conflicting pending/cleared note). Collect the $110 balance at job completion.',
