-- Lock integration authority and cross-agent change coordination.
...
on conflict(data_domain) do update set system_name=excluded.system_name,authority_level=excluded.authority_level,notes=excluded.notes,status=excluded.status,updated_at=now();
