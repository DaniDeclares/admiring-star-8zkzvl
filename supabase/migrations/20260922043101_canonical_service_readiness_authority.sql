...
SELECT rc.canonical_sku, rc.service_name, rc.division, rc.runtime_service_id, rc.service_family,
  c.governed_offer_row_count, c.commercial_offer_states, c.configuration_states,
  c.has_sell_now_offer, c.has_do_not_sell_offer,
  ...
  rc.blocking_gate, rc.release_state,
...
DO $$
DECLARE v_release bigint; v_summary bigint; v_duplicates bigint; v_live bigint;
BEGIN
 ...
 SELECT count(*) - count(DISTINCT canonical_sku) INTO v_duplicates FROM public.dd_service_canonical_readiness_v1;
 SELECT count(*) INTO v_live FROM public.dd_service_canonical_readiness_v1 WHERE production_sellable;
 IF v_release <> v_summary THEN RAISE EXCEPTION 'Canonical readiness row count % does not match release contract row count %', v_summary, v_release; END IF;
 IF v_duplicates <> 0 THEN RAISE EXCEPTION 'Canonical readiness contains % duplicate canonical SKU rows', v_duplicates; END IF;
 IF v_live <> (SELECT count(*) FROM public.dd_service_release_contract_v1 WHERE release_state='LIVE_READY') THEN RAISE EXCEPTION 'Canonical readiness LIVE count diverges from release authority'; END IF;
END $$;
