...
begin
 ...
 if v_type='LEAD' then
   ...
   if v_sales is null then
     ...
     values(nullif(v_name,''),v_phone,'INBOUND','THUMBTACK','VERIFIED','NOT_CONTACTED','Review Thumbtack request',coalesce(d#>>'{request,description}','Thumbtack service request'),'CONSUMER',v_lead,nullif(d->>'createdAt','')::timestamptz,'INBOUND',85,'HOT',false,'SUPPRESSED','THUMBTACK',false,false,jsonb_build_object('thumbtack_lead_id',v_lead,'request_id',d#>>'{request,requestID}','category',d#>>'{request,category,name}','zip_code',d#>>'{request,location,zipCode}','city',d#>>'{request,location,city}','state',d#>>'{request,location,state}','thumbtack_estimate',d#>>'{estimate,total}','raw_request',d->'request'),'THUMBTACK') returning id into v_sales;
   end if;
...
revoke all on function public.dd_process_thumbtack_event(uuid) from public,anon,authenticated;
grant execute on function public.dd_process_thumbtack_event(uuid) to service_role;
