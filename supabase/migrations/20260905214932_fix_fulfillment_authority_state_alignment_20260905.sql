-- Align fulfillment runtime with the already-approved state vocabulary.
-- Customer/request-origin work orders start at INSTANTIATED, not legacy DRAFT.
...
-- Provider approval activates compliance as VERIFIED; dispatch must use that canonical state
-- and also require the other activation gates before offering work.
...
begin
  ...
  if v_service_id is null and v_job_id is not null then
    select sr.service_id into v_service_id from public.dd_jobs j join public.service_requests sr on sr.id=j.service_request_id where j.id=v_job_id;
  end if;
  ...
  from (
    ...
    where o.is_active=true
      ...
      and upper(coalesce(o.permission_status,''))='AUTHORIZED'
  ...
  from (
    ...
    where p.is_active=true and o.is_active=true and o.accepts_new_work=true and o.capacity_status='AVAILABLE'
      and upper(coalesce(o.qualification_status,''))='QUALIFIED'
      and upper(coalesce(o.permission_status,''))='AUTHORIZED'
      and upper(coalesce(o.network_access_level,''))='AUTHORIZED'
      and upper(coalesce(o.compliance_status,''))='VERIFIED'
  ...
  if v_selected_org is not null then
    select p.id into v_selected_provider from public.dd_providers p where p.id=any(v_eligible_providers) and p.org_id=v_selected_org and p.is_active=true order by p.created_at asc,p.id limit 1;
    v_reason:='PRIMARY_PROVIDER_PRIORITY';
  elsif coalesce(array_length(v_eligible_providers,1),0)>0 then
    select p.id into v_selected_provider from public.dd_providers p join public.dd_provider_organizations o on o.id=p.org_id where p.id=any(v_eligible_providers) order by o.routing_priority asc,p.id limit 1;
  ...
  if v_job_id is not null and v_selected_org is not null and v_selected_provider is not null then
    select a.id into v_existing_offer from public.dd_job_assignments a where a.job_id=v_job_id and a.provider_org_id=v_selected_org and a.assignment_status='OFFERED' order by a.created_at desc limit 1;
    if v_existing_offer is null then
      ...
      values(v_job_id,v_selected_provider,v_selected_org,'OFFERED','Created by Provider Network resolver.',null) returning id into v_assignment_id;
      update public.dd_jobs set job_status='ASSIGNMENT_OFFERED',assigned_to=v_selected_provider,updated_at=now() where id=v_job_id;
    else v_assignment_id=v_existing_offer; end if;
  end if;
...
-- Contract task orders are staff-only contracting records; enforce the existing policy.
alter table public.dd_contract_task_orders enable row level security;

revoke all on table public.dd_contract_task_orders from anon,authenticated;
grant select,insert,update,delete on table public.dd_contract_task_orders to authenticated;
