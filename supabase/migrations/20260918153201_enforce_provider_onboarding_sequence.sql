-- Reconstructed from Supabase's own applied-migration record (supabase_migrations.schema_migrations,
-- version 20260918153201) to close a governance gap: this migration was applied directly to
-- production with no corresponding file ever committed to this repo. Content below is copied
-- verbatim from the live `statements` column, not rewritten.
--
-- Refines the two functions from 20260918152634: adds a PROVIDER_AGREEMENT_REQUIRED gate to
-- document intake (a provider can't attach documents before executing the provider agreement)
-- and tightens tax-document verification on approval from "verified or not-required" to
-- strictly "verified".
...
begin
  ...
  if p_document_type not in ('GOVERNMENT_ID','W9','COI','AUTO_INSURANCE','AGREEMENT','BUSINESS_REGISTRATION','PROFESSIONAL_LICENSE','CERTIFICATION','BACKGROUND_CONSENT','PORTFOLIO','WORK_SAMPLE','MOTOR_CARRIER_AUTHORITY','PROVIDER_PRICE_SHEET','OTHER') then raise exception 'INVALID_DOCUMENT_TYPE'; end if;
  ...
  if coalesce(v_app.agreement_status,'PENDING') <> 'EXECUTED' then raise exception 'PROVIDER_AGREEMENT_REQUIRED'; end if;
  if p_capability_id is not null and not exists (select 1 from public.dd_provider_application_capabilities c where c.id=p_capability_id and c.application_id=p_application_id) then raise exception 'CAPABILITY_NOT_FOUND_FOR_APPLICATION'; end if;

  insert into public.dd_provider_application_documents(application_id,capability_id,document_type,storage_path,verification_status,document_number,issuing_authority,expiration_date)
  values(p_application_id,p_capability_id,p_document_type,p_storage_path,'PENDING',p_document_number,p_issuing_authority,p_expiration_date)
...
declare
  ...
  v_org_id uuid; v_provider_id uuid; v_identity_id uuid;
  v_cap_count integer := 0; v_verified_cap_count integer := 0; v_provider_name text;
begin
  ...
  if coalesce(a.tax_form_status,'PENDING') <> 'VERIFIED' then raise exception 'W9_NOT_VERIFIED'; end if;
  ...
  if a.applicant_type='BUSINESS' and not exists (select 1 from public.dd_provider_application_documents d where d.application_id=a.id and d.document_type='PROVIDER_PRICE_SHEET' and d.verification_status not in ('REJECTED','EXPIRED')) then raise exception 'BUSINESS_PRICE_SHEET_REQUIRED'; end if;
  ...
  update public.dd_provider_applications set application_status='APPROVED',provider_id=v_provider_id,reviewed_at=now(),reviewed_by=p_actor_id,network_access_level='AUTHORIZED',compliance_status='VERIFIED',updated_at=now() where id=a.id;
