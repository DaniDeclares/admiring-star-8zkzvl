-- Signup recovery hardening.
-- If an older provisioning attempt consumed a staging row before the DANI portal
-- identity existed, the next authenticated login can safely reopen that
-- caller-owned row and complete provisioning. Normal failures now raise so the
-- transaction rolls back the staging claim instead of leaving a false "consumed"
-- state.
...
BEGIN
  ...
  IF v_row.kind = 'provider' AND v_row.payload ? 'providerPayload' THEN
    ...
    IF jsonb_array_length(coalesce(v_row.payload->'capabilityPayloads','[]'::jsonb)) > 0
       AND NOT EXISTS (
         SELECT 1 FROM public.dd_provider_application_capabilities
