...
create index if not exists dd_job_assignments_provider_org_idx
  on public.dd_job_assignments(provider_org_id, assignment_status);

-- Replace the resolver with an atomic routing + assignment operation. Organization-level
-- assignments are supported when no individual provider identity has been provisioned.
...
begin
  ...
  if v_selected_org is not null then
    ...
    from public.dd_providers p
    where p.org_id = v_selected_org and p.is_active = true
  ...
  if v_job_id is not null and v_selected_org is not null then
    ...
    if v_existing_offer is null then
      insert into public.dd_job_assignments (
        ...
        v_job_id, v_selected_provider, v_selected_org, 'OFFERED',
        'Created by Provider Network resolver.',
        null
  ...
  insert into private.dd_work_order_routing (
    ...
    routing_reason, assignment_id, assignment_timestamp, offer_expires_at
  ) values (
    p_request_id, v_job_id, p_service_id, p_capability_key, p_zip_code,
    v_eligible_orgs, v_eligible_providers,
    v_selected_org, v_selected_provider, v_offer_status,
    v_reason, v_assignment_id,
    case when v_assignment_id is null then null else now() end,
