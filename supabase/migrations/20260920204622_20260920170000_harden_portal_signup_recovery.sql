...
BEGIN
  ...
  -- Normal path: claim the caller's own pending staging row by exact id
  -- (confirmation link) or by normalized email (password-login recovery).
  ...
  -- Recovery path: an older implementation could mark staging consumed before
  -- returning from a handled provisioning error. If that happened, the row is
  -- still owned by this auth user but the portal identity may be missing.
  -- Re-open ONLY that caller-owned consumed row and run the same idempotent
  -- provisioning path below. This makes login a self-healing boundary.
  ...
  -- Claim exactly once. If another session already consumed the pending row,
  -- the second caller will fall through to the already-linked identity.
  UPDATE public.dd_provider_intake_staging
  ...
  -- Identity is the first DANI-side readiness contract.
  BEGIN
  ...
  ELSIF v_row.payload ? 'intakePayload' THEN
    -- A duplicate call is prevented by the portal identity existing already;
    -- this branch is reached only when this staging record is being consumed
    -- for the first time/recovery. The identity is the durable readiness key.
