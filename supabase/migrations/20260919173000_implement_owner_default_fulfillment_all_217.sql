-- Owner-default fulfillment: Danielle is the default fulfillment resource.
-- Applied live 2026-09-19 after a fresh preflight audit.
-- Scope: 217 SELL_NOW + fulfillment-gated services with zero external
-- authorized capability, excluding regulated Division 12 courier services,
-- two services explicitly assigned to other providers, and Division 13.
-- This does NOT alter pricing/economics or convert quote-first services to
-- checkout. It only establishes the owner fulfillment capability and the
-- missing channel-availability rows required by the existing runtime gate.
...
DO $$ DECLARE v_caps int; v_channels int; v_blocked int; v_quote int;
BEGIN
 SELECT count(*) INTO v_caps FROM public.dd_provider_capabilities
 ...
 SELECT count(*) INTO v_channels FROM public.dd_service_channel_availability
 ...
 SELECT count(*) INTO v_blocked FROM public.dd_service_readiness_v1
 ...
 SELECT count(*) INTO v_quote FROM public.dd_service_readiness_v1
 WHERE canonical_sku IN (SELECT canonical_sku FROM tmp_owner_fulfillment_gap)
