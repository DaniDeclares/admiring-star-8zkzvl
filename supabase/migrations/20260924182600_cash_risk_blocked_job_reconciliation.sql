-- Reconcile #395's cash-at-risk signal with the Krystal Livingston job's
-- current state.
--
-- What changed since #395 (20260924154800_job_cash_at_risk_signal.sql) was
-- authored: the job moved from job_status=scheduled/scheduled_start=today
-- 3pm to job_status=blocked/scheduled_start=null (pending an eligible
-- provider; customer approved a move to Sep 25 morning, exact time still
-- pending; a $60 flat external-provider offer exists; Cass declined).
--
-- Two incompatibilities this creates in the merged #395 migration, both
-- fixed here rather than skipped:
--
--   1. dd_compute_job_cash_risk() only considers jobs whose scheduled_start
--      falls in a lookahead window. A job that is blocked with
--      scheduled_start = null (exactly this job's current state) silently
--      drops out of the signal entirely -- the opposite of what the signal
--      is for. Fixed by also including jobs that are blocked with no
--      schedule and still carry a balance due.
--
--   2. #395's immediate dd_owner_attention_queue insert hardcoded
--      "scheduled today" language, the old 3pm/Chase framing and the old
--      $70 Thumbtack-replenishment recommended action. That text would now
--      be stale/wrong if filed as-is. This migration supersedes it with
--      language derived from the job's live status/schedule instead of
--      fixed prose, and is a superseding open item rather than a duplicate
--      (guarded on reason text, not just domain+record).
--
-- No change to the #395 columns, table, or the byte-for-byte-preserved
-- parts of dd_refresh_company_domain_state(). Additive and idempotent.
...
-- Close out #395's stale immediate item for this job (its "scheduled today"
-- framing is no longer accurate) and file a current one in its place.
