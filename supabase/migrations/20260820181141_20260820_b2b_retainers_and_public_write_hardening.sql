...
-- 1. Remove anonymous Data API access to financial/operational write surfaces.
DO $$
DECLARE
  t text;
BEGIN
...
-- Remove legacy public insert policies; server-side Prisma/API paths remain the write boundary.
...
-- 2. Estimator settings contain internal pricing/operational policy and inventory; not public API data.
...
-- 3. Replace deprecated auth.role() usage with an explicit authenticated policy target.
...
-- 4. Canonical B2B-APT enterprise retainer catalog.
