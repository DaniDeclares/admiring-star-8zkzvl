...
begin
  ...
  if p_document_type not in ('GOVERNMENT_ID','W9','COI','AUTO_INSURANCE','AGREEMENT','BUSINESS_REGISTRATION','PROFESSIONAL_LICENSE','CERTIFICATION','BACKGROUND_CONSENT','PORTFOLIO','WORK_SAMPLE','OTHER') then raise exception 'INVALID_DOCUMENT_TYPE'; end if;
  ...
  insert into public.dd_provider_application_documents (application_id,document_type,storage_path,verification_status)
  values (p_application_id,p_document_type,p_storage_path,'PENDING') returning id into v_document_id;
...
create policy provider_application_document_self_select on public.dd_provider_application_documents for select to authenticated using (exists (select 1 from public.dd_provider_applications a where a.id = dd_provider_application_documents.application_id and a.applicant_user_id = auth.uid()));

drop policy if exists provider_application_document_self_insert on public.dd_provider_application_documents;
create policy provider_application_document_self_insert on public.dd_provider_application_documents for insert to authenticated with check (exists (select 1 from public.dd_provider_applications a where a.id = dd_provider_application_documents.application_id and a.applicant_user_id = auth.uid() and a.application_status in ('DRAFT','SUBMITTED','NEEDS_INFO')));

drop policy if exists provider_onboarding_storage_self_insert on storage.objects;
create policy provider_onboarding_storage_self_insert on storage.objects for insert to authenticated with check (bucket_id = 'dd-vendor-onboarding' and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists provider_onboarding_storage_self_select on storage.objects;
create policy provider_onboarding_storage_self_select on storage.objects for select to authenticated using (bucket_id = 'dd-vendor-onboarding' and (storage.foldername(name))[1] = auth.uid()::text);
