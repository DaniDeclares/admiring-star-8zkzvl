-- Treat Danielle's explicit owner-direct fulfillment lane as a valid fulfillment path
-- when the canonical service has no unresolved required capability requirement.
-- External provider capability counts remain untouched and continue to represent the
-- external network. This is an additive runtime interpretation, not a fabricated
...
SELECT
  ...
  (s.pricing_type = ANY (ARRAY['BESPOKE_SOW','SOW','SOW_PROCUREMENT','QUOTE','STARTING_AT','CONFIGURED','VARIABLE_QUOTE']))
    OR s.starting_price IS NULL AS is_quote_required,
  EXISTS (
    SELECT 1 FROM public.dd_stripe_launch_register r
    WHERE r.canonical_sku = o.canonical_sku
      AND r.stripe_payment_link_id IS NOT NULL
  ) AS has_stripe_payment_link,
  ...
  (
    (o.division = ANY (ARRAY['01','05','07','09','10','11','12']))
    AND (COALESCE(o.ch01_a_priced,false) OR COALESCE(o.ch01_b_priced,false) OR COALESCE(o.channel_availability_count,0) > 0)
  )
  OR (
    (o.division <> ALL (ARRAY['01','05','07','09','10','11','12']))
    AND COALESCE(o.channel_availability_count,0) > 0
    AND COALESCE(o.priced_channel_count,0) > 0
  ) AS channel_ok,
  ...
  CASE
    ...
    WHEN NOT (
      COALESCE(o.authorized_provider_capability_count,0) > 0
      OR (
        EXISTS (
          SELECT 1 FROM public.dd_master_service_universe m
          WHERE m.canonical_sku=o.canonical_sku
            ...
            AND NOT EXISTS (
              SELECT 1 FROM public.dd_service_capability_requirements r
              WHERE r.canonical_sku=o.canonical_sku AND r.required=true
            )
        )
      )
    ) THEN 'NO_FULFILLMENT_CAPABILITY'
    WHEN (s.pricing_type = ANY (ARRAY['BESPOKE_SOW','SOW','SOW_PROCUREMENT','QUOTE','STARTING_AT','CONFIGURED','VARIABLE_QUOTE']))
      OR s.starting_price IS NULL THEN 'LIVE_QUOTE_ONLY'
    WHEN NOT (
      ((o.division = ANY (ARRAY['01','05','07','09','10','11','12']))
        AND (COALESCE(o.ch01_a_priced,false) OR COALESCE(o.ch01_b_priced,false) OR COALESCE(o.channel_availability_count,0) > 0))
      OR
  ...
  CASE
    ...
    ) THEN 'MISSING_AUTHORIZED_PROVIDER_CAPABILITY'
    WHEN (s.pricing_type = ANY (ARRAY['BESPOKE_SOW','SOW','SOW_PROCUREMENT','QUOTE','STARTING_AT','CONFIGURED','VARIABLE_QUOTE']))
      OR s.starting_price IS NULL THEN 'QUOTE_FIRST_BY_DESIGN'
    WHEN NOT (
      ((o.division = ANY (ARRAY['01','05','07','09','10','11','12']))
        AND (COALESCE(o.ch01_a_priced,false) OR COALESCE(o.ch01_b_priced,false) OR COALESCE(o.channel_availability_count,0) > 0))
      OR
