-- DANI DECLARES: economic release gate alignment.
-- The application checkout gate blocks unresolved economics; release readiness must do the same.
-- This preserves the existing release contract columns while adding ECONOMICS as a blocking state.
CREATE OR REPLACE VIEW public.dd_service_release_contract_v1 WITH (security_invoker=true) AS
...
economics_rollup as (
 select distinct on (canonical_sku) canonical_sku, internal_cost, margin_economics
 from public.dd_master_service_universe
 where lifecycle_status='CANONICAL_ACTIVE'
 order by canonical_sku, updated_at desc
),
...
provider_rollup as (
 select service_id,count(*) filter(where is_authorized=true) provider_capability_count
 from public.dd_provider_capabilities group by service_id
),
routing_rollup as (select service_id,count(*) routing_count from private.dd_work_order_routing group by service_id),
task_rollup as (
 select service_id,
        count(*) active_task_template_count,
        count(*) filter(where evidence_required=true) active_evidence_task_count
...
select
 ...
 (coalesce(f.required_requirement_count,0)>0 and coalesce(pr.provider_capability_count,0)>0 and coalesce(r.routing_count,0)>0 and coalesce(t.active_task_template_count,0)>0 and coalesce(t.active_evidence_task_count,0)>0) fulfillment_matrix_ok,
 ...
 case
  ...
  when not((
  coalesce(nullif(trim(e.internal_cost),''),'') <> ''
  AND coalesce(nullif(trim(e.margin_economics),''),'') <> ''
  AND e.internal_cost !~* 'PENDING_RECONCILIATION|DRAFT|NOT AN AUDITED|NEEDS DANIELLE|PRICE MISMATCH|FLAGGED, NOT RESOLVED|PROVISIONAL|PLANNING ASSUMPTION'
  AND e.margin_economics !~* 'PENDING_RECONCILIATION|DRAFT|NOT AN AUDITED|NEEDS DANIELLE|PRICE MISMATCH|FLAGGED, NOT RESOLVED|PROVISIONAL|PLANNING ASSUMPTION'
  AND coalesce(nullif(substring(e.margin_economics from '(-?[0-9]+(\\.[0-9]+)?)\\s*%'),''),'0')::numeric >= 50
)) then 'ECONOMICS'::text
  ...
  when not(coalesce(f.required_requirement_count,0)>0 and coalesce(pr.provider_capability_count,0)>0 and coalesce(r.routing_count,0)>0 and coalesce(t.active_task_template_count,0)>0 and coalesce(t.active_evidence_task_count,0)>0) then 'FULFILLMENT_MATRIX'
 ...
 case
  ...
  AND coalesce(nullif(substring(e.margin_economics from '(-?[0-9]+(\\.[0-9]+)?)\\s*%'),''),'0')::numeric >= 50
   ...
   and coalesce(r.routing_count,0)>0 and coalesce(t.active_task_template_count,0)>0 and coalesce(t.active_evidence_task_count,0)>0
