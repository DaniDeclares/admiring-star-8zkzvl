...
BEGIN
  ...
  SELECT count(*) INTO v_pricing_rows
  FROM public.dd_service_pricing_rules
  WHERE service_id=v_service_id;

  IF v_pricing_rows <> 5 THEN
    RAISE EXCEPTION 'DNI-01B-008 expected 5 channel pricing rules, found %', v_pricing_rows;
  END IF;

  SELECT count(*) INTO v_channel_rows
  FROM public.dd_service_channel_availability
  WHERE service_id=v_service_id
    AND channel_code IN ('CH01','CH02','CH03','CH04','CH05');

  IF v_channel_rows <> 5 THEN
    RAISE EXCEPTION 'DNI-01B-008 expected 5 channel availability rows, found %', v_channel_rows;
  END IF;

  IF EXISTS (
    SELECT 1 FROM public.dd_service_pricing_rules
    WHERE service_id=v_service_id
      AND (pricing_type <> 'STARTING_AT'
        OR base_price_cents <> 3500
        OR currency <> 'USD'
        OR billing_cycle <> 'ONETIME'
        OR lock_status <> 'PENDING'
        OR status <> 'PENDING_RECONCILIATION')
  ) THEN
    RAISE EXCEPTION 'DNI-01B-008 pricing rules do not match the reconciled governed state';
  END IF;
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'DNI-01B-008 channel rows are not fail-closed PENDING';
  END IF;

  RAISE NOTICE 'DNI-01B-008 reconciliation verified: 5 pricing rules + 5 PENDING channel rows.';
END $$;
