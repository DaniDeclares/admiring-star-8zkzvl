...
begin
  if exists (
    select 1 from public.dd_ch02_service_adjudication
    where source_master_record_id is null
  ) then
...
alter table public.dd_ch02_service_adjudication
  ...
  check (
    (disposition = 'FRONT_DOOR_CANDIDATE' and proposed_front_door in ('TURNOVER_MAKE_READY','PROPERTY_RESCUE_FIELD_DISPATCH','PROPERTY_CONDITION_DOCUMENTATION','OFFICE_OPERATIONS_RESCUE'))
