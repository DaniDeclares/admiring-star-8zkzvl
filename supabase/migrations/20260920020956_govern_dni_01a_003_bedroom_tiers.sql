...
DECLARE
  v_service_id uuid;
BEGIN
  SELECT id INTO v_service_id FROM public.services WHERE sku = 'DNI-01A-003';
  IF v_service_id IS NULL THEN RAISE EXCEPTION 'DNI-01A-003 not found'; END IF;
  ...
  SET base_price_cents = 33000,
      ...
      lock_status = 'LOCKED',
      status = 'ACTIVE'
  WHERE service_id = v_service_id AND status = 'ACTIVE';

  IF (SELECT count(*) FROM public.dd_service_pricing_rules WHERE service_id = v_service_id AND status = 'ACTIVE') <> 5 THEN
