...
begin
  ...
  select exists (
    select 1 from public.dd_provider_capabilities c
    where c.provider_id=v_provider_id and c.is_authorized=true
      and c.capability_key in ('AP_AR_ADMIN','BOOKKEEPING_SETUP','CASH_FLOW_BUDGETING','FINANCIAL_READINESS','FINANCIAL_REPORTING','MONTHLY_BOOKKEEPING')
  ) into v_authorized;
  ...
  select jsonb_build_object(
    ...
    'lanes',coalesce((select jsonb_agg(jsonb_build_object(
      'capabilityKey',r.capability_key,'lane',r.dashboard_lane,'routeMode',r.route_mode,
      'ownerEscalationRule',r.owner_escalation_rule,'externalWorkMode',r.external_work_mode,
      'internalWorkMode',r.internal_company_work_mode,'compensationMode',r.internal_compensation_mode
    ) order by r.dashboard_lane) from public.dd_provider_dashboard_lane_routes r
      where r.provider_id=v_provider_id and r.is_active=true),'[]'::jsonb),
    'exceptions',coalesce((select jsonb_agg(to_jsonb(x) order by x.created_at desc) from (
      select id,exception_type,source_system,source_record_type,source_record_id,description,
             assigned_lane,status,requires_owner_decision,resolution,resolved_at,metadata,created_at,updated_at
      from public.dd_accounting_exception_queue
      where status not in ('RESOLVED','CLOSED')
      order by created_at desc limit 100
    ) x),'[]'::jsonb),
    'sources',coalesce((select jsonb_agg(to_jsonb(x) order by x.updated_at desc) from (
      select id,source_key,source_name,system_name,classification,coverage_period,accounting_treatment,
             overall_status,found_status,extracted_status,reconciled_status,qbo_posted_status,
             source_reference,notes,last_verified_at,updated_at
      from public.dd_accounting_source_register
      order by updated_at desc limit 100
    ) x),'[]'::jsonb),
    'payables',coalesce((select jsonb_agg(to_jsonb(x) order by x.accrued_at desc) from (
      select id,work_order_id,provider_id,base_payout_amount,travel_allowance,approved_change_order_addition,
             total_final_payable,is_cleared_for_payout,payment_reference_id,accrued_at,settled_at
      from public.dd_accounts_payable_ledger
      order by accrued_at desc limit 100
    ) x),'[]'::jsonb),
    'reviews',coalesce((select jsonb_agg(to_jsonb(x) order by x.reviewed_at desc) from (
      select id,source_type,source_id,review_status,reviewer_note,proposed_treatment,reviewed_at
      from public.dd_accounting_work_reviews where provider_id=v_provider_id
...
create or replace function public.dd_review_my_accounting_item(
  p_source_type text,
  p_source_id text,
  p_review_status text,
  p_reviewer_note text default null,
  p_proposed_treatment jsonb default '{}'::jsonb
)
returns jsonb
language plpgsql
security definer
...
declare
  v_provider_id uuid;
  v_authorized boolean;
  v_exception public.dd_accounting_exception_queue%rowtype;
  v_review public.dd_accounting_work_reviews%rowtype;
begin
  if auth.uid() is null then raise exception 'AUTH_REQUIRED'; end if;
  v_provider_id := public.dd_current_provider_id();
  if v_provider_id is null then raise exception 'PROVIDER_IDENTITY_REQUIRED'; end if;
  ...
  if p_source_type='ACCOUNTING_EXCEPTION' then
    ...
    if p_review_status='RESOLVED_NON_OWNER' then
      update public.dd_accounting_exception_queue
      set status='RESOLVED', resolution=coalesce(nullif(trim(p_reviewer_note),''),'Resolved by authorized accounting provider review'),
          resolved_by='accounting_provider:'||v_provider_id::text,resolved_at=now(),updated_at=now()
      where id=v_exception.id;
    elsif p_review_status='NEEDS_OWNER' then
      update public.dd_accounting_exception_queue
      set status='PENDING_OWNER',requires_owner_decision=true,updated_at=now()
      where id=v_exception.id;
    end if;
  end if;
  ...
  values(v_provider_id,p_source_type,p_source_id,p_review_status,nullif(trim(p_reviewer_note),''),coalesce(p_proposed_treatment,'{}'::jsonb),auth.uid())
