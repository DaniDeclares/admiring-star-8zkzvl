-- DANI research lead worker + deterministic promotion gate
-- Mirrors production migration applied 2026-09-24 and registers the governed worker.
...
begin
  ...
  where (v_email is not null and lower(trim(coalesce(q.email,'')))=v_email)
     ...
     or (lower(trim(coalesce(q.company_name,'')))=v_company
         and coalesce(q.sales_metadata->>'market','')=coalesce(v_lead.market,''))
  order by q.updated_at desc limit 1;

  if found then
    update public.dd_research_leads
      set promotion_status='MATCHED_EXISTING', promoted_sales_queue_id=v_existing.id,
  ...
  insert into public.dd_sales_queue(
    ...
    null,coalesce(v_lead.verified_company_name,v_lead.company_name),
    nullif(trim(v_lead.verified_phone),''),v_email,'WARM','WEB_SOURCED','VERIFIED',
    ...
    jsonb_build_object(
      'research_lead_id',v_lead.id,'market',v_lead.market,
      'research_priority',v_lead.research_priority,'research_claims',v_lead.research_claims,
  ...
  update public.dd_research_leads
    set promotion_status='PROMOTED', promoted_sales_queue_id=v_sales_id,
        promoted_at=now(), updated_at=now()
    where id=v_lead.id;
...
returns jsonb language plpgsql security definer set search_path = '' as $$
declare
  v_row record; v_result jsonb; v_promoted integer := 0; v_matched integer := 0; v_blocked integer := 0;
begin
  for v_row in
    select id from public.dd_research_leads
    where verification_status='VERIFIED' and promotion_status='RESEARCH_ONLY'
    order by updated_at asc limit greatest(1,least(coalesce(p_limit,25),100))
  loop
...
select cron.schedule('promote-verified-research-leads','*/15 * * * *',
  $$select private.dd_promote_verified_research_leads_batch(25);$$)
where not exists (select 1 from cron.job where jobname='promote-verified-research-leads');

insert into public.dd_revenue_agent_registry(
  agent_key,agent_name,responsibility,allowed_actions,prohibited_actions,is_active,updated_at
) values (
  'DANI_LEAD_SCOUT','DANI Lead Scout',
  'Research-only prospect verification worker. Verifies public company/contact facts with provenance and hands eligible records to the deterministic promotion gate.',
  '["read_research_staging","verify_public_company_identity","verify_public_business_contact","record_source_evidence","submit_verified_research_facts"]'::jsonb,
  '["send_outreach","infer_consent","change_pricing","create_quote","change_job_state","authorize_provider","change_accounting","bypass_promotion_gate"]'::jsonb,
  true,now()
)
