...
create index if not exists idx_dd_invoices_stripe_invoice_id on public.dd_invoices(stripe_invoice_id) where stripe_invoice_id is not null;
create index if not exists idx_dd_invoices_estimate_stripe on public.dd_invoices(estimate_id, created_at desc);
