...
on conflict (channel_code,cost_pool,category,source_type) do update set lower_percent=excluded.lower_percent,upper_percent=excluded.upper_percent,benchmark_percent=excluded.benchmark_percent,source_reference=excluded.source_reference,source_date=excluded.source_date,governance_status=excluded.governance_status,notes=excluded.notes,updated_at=now();
