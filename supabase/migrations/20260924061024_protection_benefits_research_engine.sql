...
insert into public.dd_research_programs(program_key,program_name,domain,objective,status,release_blocked,green_rule,metadata) values
('PROTECTION_BENEFITS','Protection & Benefits','INSURANCE_LEGAL_BENEFITS','Build a governed DANI insurance, legal-plan and employee-benefits distribution capability without exposing customers to unlicensed or unverified sales.','RESEARCHING',true,'Credential + compliance + economics + distribution + ownership/renewal + fulfillment evidence must all be GREEN before customer-facing release.','{"channels":["CH01","CH02","CH03","CH04","CH05"],"customer_release":"BLOCKED","research_mode":"continuous"}'::jsonb)
