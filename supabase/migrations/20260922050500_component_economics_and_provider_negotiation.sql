...
create table if not exists public.dd_component_cost_baselines (
  id uuid primary key default gen_random_uuid(),
  component_id uuid not null references public.dd_service_components(id) on delete cascade,
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint dd_component_cost_type_chk check (cost_type in ('OWNER_LABOR','PROVIDER_LABOR','MATERIAL','PROCUREMENT','SUBCONTRACT','TRAVEL','PAYMENT_PROCESSING','OTHER')),
  constraint dd_component_cost_status_chk check (status in ('DRAFT','ACTIVE','PAUSED','RETIRED')),
