...
begin
  ...
  if not actor_is_staff and coalesce(j.assigned_to,'') <> actor_provider::text then raise exception 'JOB_ACTOR_UNAUTHORIZED'; end if;
  ...
  values(j.id,actor_id,'START_JOB','Field execution started.',
    jsonb_build_object('next_status','in_progress','work_order_id',j.work_order_id,'telemetry_source','APPLICATION_PORTAL_ACTION'));
  return j;
end;
$$;
...
begin
  ...
  insert into public.dd_dispatch_events(job_id,actor_id,event_type,description,metadata)
  values(j.id,actor_id,'COMPLETE_JOB','Field execution completed and submitted for QA.',
    jsonb_build_object('previous_status','in_progress','next_status','submitted','work_order_id',j.work_order_id,'telemetry_source','APPLICATION_PORTAL_ACTION'));
  return j;
end;
$$;
