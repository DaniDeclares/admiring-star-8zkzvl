...
CREATE TABLE public.dd_service_economic_baselines (
  ...
  CONSTRAINT dd_service_economic_baselines_evidence_status_chk CHECK (evidence_status IN ('THEORETICAL_BASELINE','OBSERVED_JOB','AUDITED')),
  CONSTRAINT dd_service_economic_baselines_duration_chk CHECK (estimated_duration_hours IS NULL OR estimated_duration_hours > 0),
  CONSTRAINT dd_service_economic_baselines_labor_rate_chk CHECK (labor_rate >= 0),
  CONSTRAINT dd_service_economic_baselines_materials_chk CHECK (materials_cost >= 0),
  CONSTRAINT dd_service_economic_baselines_travel_chk CHECK (travel_cost IS NULL OR travel_cost >= 0),
  CONSTRAINT dd_service_economic_baselines_other_direct_chk CHECK (other_direct_cost >= 0),
  CONSTRAINT dd_service_economic_baselines_travel_status_chk CHECK (travel_status IN ('NOT_REVIEWED','PENDING_REAL_WORLD_MILEAGE','NOT_APPLICABLE_REMOTE_BASELINE','OBSERVED','AUDITED')),
  CONSTRAINT dd_service_economic_baselines_theoretical_policy_chk CHECK (evidence_status <> 'THEORETICAL_BASELINE' OR labor_rate = 30.00)
);
...
CREATE INDEX dd_service_economic_baselines_master_service_idx ON public.dd_service_economic_baselines(master_service_id);
CREATE INDEX dd_service_economic_baselines_runtime_service_idx ON public.dd_service_economic_baselines(runtime_service_id);
CREATE INDEX dd_service_economic_baselines_evidence_status_idx ON public.dd_service_economic_baselines(evidence_status);
...
SELECT
  ...
  0.00,
  CASE WHEN e.sku LIKE 'DNI-04A-%' THEN 'NOT_APPLICABLE_REMOTE_BASELINE' ELSE 'PENDING_REAL_WORLD_MILEAGE' END,
...
SELECT
  ...
  CASE WHEN p.canonical_sku LIKE 'DNI-04A-%' THEN 'REMOTE_OR_BLOCK_SCOPED' ELSE 'FIELD_TRAVEL_NOT_INCLUDED_IN_THIS_SCENARIO' END AS travel_treatment,
